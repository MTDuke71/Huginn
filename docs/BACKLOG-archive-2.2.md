# BACKLOG archive — 2.2/2.3 era (t25 → t34)

Full detail for every item **closed** during the Huginn 2.2/2.3 development
arc: shipped baselines t25 through t34, the 2026-07-09 independent code
audit (#52–#61, criticals + build/test safety net), the SF18-gap-study
selectivity program (#62 singular extensions, #17-r2 aspiration re-ship,
#63 history-modulated LMR), #42 TT aging/clusters, #31's hash-size sweep,
and the complete road-to-v2.3 release checklist (items 1–6, including the
final Stash recalibration and the UCI hygiene fixes it surfaced).

For issues #1–#37 (the 2.0-era history), see
[BACKLOG-archive-2.0.md](BACKLOG-archive-2.0.md). For the 2.1/2.2-era
history (t16→t24), see [BACKLOG-archive-2.1.md](BACKLOG-archive-2.1.md).
For currently open items, see [BACKLOG.md](BACKLOG.md). Issue numbers are
preserved so cross-references from any file still resolve. Full baseline
writeups (ship signatures, SPRT legs) live in
[BASELINE_LADDER.md](BASELINE_LADDER.md); the shipped-feature ladder for
this era is summarized in [CHANGELOG_2.3.md](CHANGELOG_2.3.md).

## Quick index

| # | Title | Status |
|---|-------|--------|
| — | Road to v2.3 release checklist (items 1–6) | **COMPLETE** — see below |
| 52 | Check-aware qsearch + horizon terminal states | **SHIPPED** → `baseline-t26` |
| 53 | Rule-50-aware TT eligibility (#29 follow-up) | **SHIPPED** → `baseline-t26` |
| 54 | Transactional, bounded FEN / `position` input | **CLOSED** (unconditional) |
| 55 | Bound every fixed-capacity move-list write | **CLOSED** (unconditional) |
| 56 | UCI parser, options, timing, and search-control contract | **CLOSED** (parts 1–3) |
| 57 | Use legal-move ordinal for PVS / LMR | **SHIPPED** → `baseline-t27` |
| 58 | Make SEE sound before using it for hard pruning | **SHIPPED** → `baseline-t28` |
| 59 | En-passant key semantics (repetition + Polyglot) | **SHIPPED** → `baseline-t29` |
| 60 | Make CMake / CTest / CI a trustworthy safety net | **CLOSED** (core + parser-purity/randomized invariants) |
| 61 | Repair or remove divergent public helper APIs | **CLOSED** |
| 62 | Singular extensions (SF18-study EBF lever) | **SHIPPED** → `baseline-t31` |
| 17-r2 | Aspiration windows at the root (re-test) | **SHIPPED** → `baseline-t32` |
| 63 | History-modulated LMR | **SHIPPED** → `baseline-t33` |
| 42 | TT aging + clusters (Fruit/Toga design) | **CLOSED** — idea 1 shipped `baseline-t34`, idea 2 parked |
| 31 | TT-size (`Hash`) sweep | **CLOSED** — flat on both boxes, default stays 64MB |
| 9 / 35 | Threats round 2 (eval, this era's slice) | **SHIPPED** → `baseline-t30`; roadmap continues in BACKLOG.md |
| 7 | Late move pruning re-test | **RE-PARKED** — two-machine sign-split, dead flat |

---

## Road to v2.3 (release checklist — COMPLETE)
...
**Item 5 (hygiene, no gauntlet needed) — DONE:**
   - **Tactical sweep on the t34 build: DONE (2026-07-15)** — WAC300 @
     ...
     **LCT2: DONE (2026-07-15)** — first-ever run (new `test/lct2_test.py`
     ...
     chased, kept as a reference pool for a future eval/search round. Full
     writeup + table in
   ...
   - **`CHANGELOG_2.3.md`: DRAFTED (2026-07-15), finalized (2026-07-16)** —
     full t22 → t34 ladder, one row per baseline with the shipped feature +
     result, mirroring [CHANGELOG_2.2.md](CHANGELOG_2.2.md)'s format, plus
     the WAC300/LCT2/`#41` corroborating evidence and the final
     recalibration numbers. See [CHANGELOG_2.3.md](CHANGELOG_2.3.md).
   - `#37` (board-desync) stayed open throughout — guarded at the UCI
     boundary, did not gate the release (still open, see BACKLOG.md).

**Item 6 (`#5` recalibration → tag `v2.3`) — DONE (2026-07-16, AMD box):**
   - Release candidate = `t34` + the item-5 UCI-only changes (search-
     behavior-identical, verified byte-for-byte: startpos d14 = 3,481,582 /
     cp 31 / e2e4 unchanged) + `id name Huginn 2.3`. Three anchors run
     (`test_huginn_gauntlet.bat`, cc=1, 10+0.1, no book):
     - Stash 19.0 (2473): **+132.28 ± 33.41**, 68.17% (300g, user-stopped) → pin ~2605.
     - Stash 20.0.1 (2509): **+93.95 ± 26.75**, 63.20% (500g, full) → pin ~2603.
     - Stash 21.0 (2714): **−38.37 ± 33.22**, 44.50% (300g, user-stopped) → pin ~2676.
   - **Inverse-variance pool of all three: ~2625 ± 18, reported as a
     ~2600–2680 range** rather than a false-precision point — 19/20 agree
     within 2 Elo of each other, but 21 (a different engine generation, not
     just the next rung) pins ~72 Elo higher, a real lean (~1.8 combined
     SE) rather than noise. Net gain vs `t26`'s 2571 ± 19: **≈ +33 to +54**,
     well under the naive ≈ +90 estimate this checklist originally
     projected (t27→t34 self-play addend ≈ +105-122 at t26's ~0.86
     compression) — this ladder's selectivity-heavy gains compressed
     harder (~0.3–0.4) going external. This also **reproduces `#5`'s
     2026-06-15 "score compression / monotonic anchor drift" finding** on a
     completely different, higher-strength anchor pair (see `#5` in
     BACKLOG.md). Full writeup: [BASELINE_LADDER.md](BASELINE_LADDER.md).
   - ⚠ Saturation check: Stash 20.0.1 (the pre-registered trigger) came in
     at 63.20% — **under** the ~65% line, so the rule wasn't formally
     triggered (Stash 19 alone crossing it is expected for the weaker of
     two anchors, not itself a red flag).
   - **Stash 21.2 (2785) not run this session** — wired into the bat
     (`stash212`) for next time. **Going forward: drop Stash 19, use
     20/21/21.2 as the three-anchor bracket** (user call, 2026-07-16).
   - **Tagged `v2.3` (2026-07-16).** Release-build snapshot tracked live in
     BACKLOG.md, not archived (routine per-machine step).
...
it too); stand-pat remains the approximation there. Test-arm startpos d14 =
...
ever observed). **Intel leg (2026-07-10): mild REGRESSION, not neutral — −18.08 ± 15.32,
...
`ENABLE_SANITIZERS` currently adds no flags at all. **Follow-up (2026-07-15,
road-to-v2.3 item 5):** the position-rejection diagnostics this item added
were gated behind `debug_mode`, so a rejected command gave the caller zero
signal — found via a malformed castling flag in an LCT2 EPD entry; fixed to
always print as `info string` (see `#60`'s parser-purity note).
...
the SPRT per house process. **AMD leg (2026-07-11):
...
**Resolution:** the EP right is normalized at the source — `MakeMove` and
`set_from_fen` store it only when a side-to-move pawn can pseudo-capture onto
it (Polyglot / X-FEN convention). Full + incremental zobrist, repetition, TT,
`to_fen`, and Polyglot all inherit the semantics; verified against the
Polyglot spec anchors. Signature: startpos d14 = 5,485,978 / cp 26 / e2e4
(−23% vs t28's 7,128,502 — decorative EP no longer splits transpositions).
**SHIPPED (2026-07-11, `baseline-t29`)** — unconditional; AMD regression gate
clean, +8.34 ± 15.32, LOS 85.73%, 1000g. Full writeup:
[BASELINE_LADDER.md](BASELINE_LADDER.md).
...
**Resolution (parser-purity + randomized invariants, 2026-07-15, road-to-v2.3
item 5): DONE.** `test_uci_go_parsing.cpp` / `test_uci_position_parsing.cpp`
no longer wrap a real search/`EXPECT_NO_THROW` — `handle_go`'s token parsing
was extracted into a pure, side-effect-free `parse_go_command`
(`uci_utils.cpp`) so tests assert the exact resulting `MinimalLimits`;
position tests assert the exact resulting FEN via `current_position()` (the
`#54` pattern), including a case for the LCTII.CMB.06 malformed-castling-flag
bug class found this session (see `#54`). New `test_randomized_invariants.cpp`:
fixed-seed random legal-move walks (5 structurally varied seed positions incl.
Kiwipete and near-promotion endgames, 200 walks × 40 plies) asserting
`is_consistent()` and exact FEN+Zobrist restoration at every unwound ply, plus
a `MakeNullMove`/`TakeNullMove` no-op check wherever legal. (Caught one bug
along the way — in the test itself, not the engine: an invalid
`uniform_int_distribution` range when a walk hit checkmate/stalemate before
checking move count.) 295/295 tests green (1 by-design skip). **Item now
CLOSED except:** exact eval-mirror properties are covered by
`test_evaluation_symmetry.cpp` but only over 8 hand-picked positions, not
randomized — a natural extension of the same pattern, not a currently known
gap (tracked as a low-priority note, not a live BACKLOG item).
...
### #9 / #35 (this era's slice): Threats round 2 — SHIPPED `baseline-t30`

Extends the +54 t15 cluster: hanging pieces (attacked *and* undefended),
pawn-push threats, threat-by-king. Same machinery, proven. **CANDIDATE
(2026-07-12) behind `ENABLE_THREATS_R2`** (default OFF, flag-off
byte-identical to t29 — startpos d14 = 5,485,978 unchanged). Three classes
layered on t15: hanging units (attacked + undefended), safe pawn-push
threats, hanging units in the king ring. Params Texel-fitted `--only-new`
(new tuner mode: the six new params fit while the rest of the 841-param
vector stays frozen — clean SPRT attribution): HANGING 9/18, PAWN_PUSH 10/4,
BY_KING −5/34; **new-feature ΔMSE 0.056857 → 0.056459 (−0.000398, ~1.6× the
t16 KS round)**. Bake-verified exact. Test-arm sigs: startpos d14 = 8,298,375
/ cp 26 / e2e4 (+51% — big tree reshape, fixed time is the SPRT's call);
Kiwipete d13 = 1,846,915 / cp −83 / e2a6. 7 regression tests
(`test_eval_threats_r2.cpp`): mirror symmetry both arms, term behaviour +
unsafe-push filter on the ON arm. Both arms full suite green. **Two-machine
SPRT vs t29 — both legs positive:** AMD +9.73 ± 14.88 (LOS 90.02%, 1000g cap,
LLR 0.75) / Intel +24.01 ± 14.55 (**LOS 99.94%**, 1000g cap, LLR 2.54), both
clean 1h48–49m runs. **Pooled ≈ +17.0 ± 10.4 Elo, LOS ≈ 99.9%, 2000g, 52.43%**
(W552/L455/D993, Ptnml [44,218,406,261,71]) — neither leg crossed the LLR
bound before the cap but the pool clears the strict 95% bar decisively.
**SHIPPED `baseline-t30` (2026-07-12)** — largest eval-term ship since t15;
`ENABLE_THREATS_R2` default flipped ON (source `#ifndef` + CMake option).
Ship build verified startpos d14 = 8,298,375 / cp 26 / e2e4; 265 tests
green (1 by-design skip); huginn_t30 snapshotted. Run-sheet in
[SPRT_QUEUE_TEST_PLAN.md](SPRT_QUEUE_TEST_PLAN.md); writeup in
[BASELINE_LADDER.md](BASELINE_LADDER.md). The rest of the eval-program
roadmap (passed-pawn refinements, lower-priority items) remains open — see
`#9 / #35` in [BACKLOG.md](BACKLOG.md).

### #42: TT aging + clusters (Fruit/Toga design) — CLOSED
...
Paired with #31's Hash sweep (test aging + size together).

### #31: TT-size (`Hash`) sweep — CLOSED, flat on both boxes (road-to-2.3 item 4)

Three-engine LTC round-robin (`test_huginn_hash_sweep.bat`): one
`huginn_t34.exe` × Hash 64/256/1024, 60+0.6, 600g/leg, run on BOTH boxes
(7800X3D's 96MB L3 V-Cache vs Intel — TT probing is memory-latency-bound,
optimal size may differ per architecture; PEXT precedent).

**AMD (7800X3D, 400g/engine, 6h27m):** h256 +2.61 ± 21.49 / h1024 0.00
(anchor) / h64 −2.61 ± 21.89. **Intel (600g, 6h25m):** h1024 +6.95 ± 21.95
/ h64 +4.34 ± 21.49 / h256 −11.30 ± 22.78. Every arm sits inside its own CI
on both boxes, and the two boxes don't even agree on a direction (AMD ranks
h256>h1024>h64, Intel ranks h1024>h64>h256) — that's noise, not a real
per-architecture split. No clear LTC ordering on either machine ⇒ per the
pre-registered rule, **UCI release default stays at 64MB**
([uci.cpp:250](../src/uci.cpp#L250)); gauntlet standard unchanged.

### #7: Late move pruning re-test — RE-PARKED (road-to-2.3 item 3)

Intel blitz leg negative lean (−6.60 ± 15.09, LOS 19.54%, 1000g; no AMD leg,
cannot pool to the bar). Two-era verdict: −254→−56 vs t1, −6.6 vs t34 — five
ordering ships moved LMP toward zero but never across; Huginn's shallow tree
is already thin (RFP/null/razoring/futility/history-LMR). In-tree behind
`ENABLE_LMP` (default OFF); a future revisit needs a genuinely new ordering
signal, not dialing (see `#7` in [BACKLOG.md](BACKLOG.md)'s deferred list for
the live status).
