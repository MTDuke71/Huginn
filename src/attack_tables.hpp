...
/**
 * @brief Check if a piece on 'from' square attacks 'to' square (non-sliding pieces only)
 * @param piece_type The type of piece (Knight, King, or Pawn)
 * @param color The color of the piece (only relevant for pawns)
 * @param from The square the piece is on
 * @param to The target square
 * @return true if the piece attacks the target square
 */
inline bool piece_attacks_square(PieceType piece_type, Color color, int from, int to) {
    uint64_t target_bit = 1ULL << to;
    
    switch (piece_type) {
        case PieceType::Knight:
            return (knight_attacks[from] & target_bit) != 0;
        case PieceType::King:
            return (king_attacks[from] & target_bit) != 0;
        case PieceType::Pawn:
            return (pawn_attacks[static_cast<int>(color)][from] & target_bit) != 0;
        default:
            return false; // Sliding pieces need occupancy information
    }
}

/**
 * @brief Get attack bitboard for a non-sliding piece
 * @param piece_type The type of piece (Knight, King, or Pawn)
 * @param color The color of the piece (only relevant for pawns)
 * @param square The square the piece is on
 * @return Bitboard of squares the piece attacks
 */
inline uint64_t get_piece_attacks(PieceType piece_type, Color color, int square) {
    switch (piece_type) {
        case PieceType::Knight:
        ...
        case PieceType::Pawn:
            return pawn_attacks[static_cast<int>(color)][square];
        default:
            return 0ULL; // Sliding pieces need occupancy information
    }
}