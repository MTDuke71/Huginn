...
/// @brief (File, Rank) -> square64 index (a1=0, h1=7, a8=56, h8=63).
constexpr inline int sq64(File f, Rank r) {
    return static_cast<int>(f) + static_cast<int>(r) * 8;
}

/// @brief File of a square64, or File::None if @p sq is off-board (outside 0-63).
constexpr inline File file_of_sq64(int sq) {
    return (sq >= 0 && sq < 64) ? File(sq & 7) : File::None;
}

/// @brief Rank of a square64, or Rank::None if @p sq is off-board (outside 0-63).
