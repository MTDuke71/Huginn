@echo off
REM ===========================================================================
REM AMD-machine variant of test_huginn_vs_t5.bat
REM
...
REM   - Run THIS bat on the AMD box  -> 200 games -> huginn_vs_t5_amd.pgn
REM   - Run test_huginn_vs_t5.bat on the Intel box -> 200 games -> huginn_vs_t5_intel.pgn
REM   - Pool both PGNs (same engine names "Huginn_current" / "Huginn_t5",
REM     same frozen t5 baseline binary, same tc, same concurrency) for a
REM     combined 400-game Elo estimate.
REM
REM Regression target: huginn_t5.exe == commit 3eab266 (BACKLOG #24 ship,
REM "git tag baseline-t5"): t4 stack + P1a (#1) + TT bound fix (#23) +
REM real magic bitboards (#24) = ~+75 Elo over baseline-t4 (LOS 99.99% /
REM single Intel 200g, 2026-05-16).
REM
...
REM probe behaviour matches the TB-enabled t5 baseline binary.
REM
REM Usage: test_huginn_vs_t5_amd.bat [rounds]   (default: 100 rounds = 200 games)
REM ===========================================================================
...
if exist "%RESULTS%\huginn_vs_t5_amd.pgn" del /q "%RESULTS%\huginn_vs_t5_amd.pgn"
if exist "%RESULTS%\fastchess_t5_amd.log" del /q "%RESULTS%\fastchess_t5_amd.log"

echo.
echo [AMD 7800X3D] Running %ROUNDS% rounds (= %ROUNDS%*2 games): Huginn current vs huginn_t5 tc=10+0.1
echo.
...
"%FASTCHESS%" ^
  ...
  -pgnout file="%RESULTS%\huginn_vs_t5_amd.pgn" notation=san append=false ^
  -log file="%RESULTS%\fastchess_t5_amd.log" level=warn
