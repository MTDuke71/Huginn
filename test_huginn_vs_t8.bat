...
REM huginn_t8.exe (frozen baseline-t8).
REM
REM baseline-t8 = baseline-t7 + BACKLOG #15 ply-tracked counter-move heuristic
REM   enabled @ order-score 1500 (commit b9d63f8). Pooled 2000g vs t7:
REM   +7.12 Elo [-3.2,+17.4], LOS 91.24%, W479/L438/D1083 -- Intel +7.30 /
REM   AMD +6.95, tight cross-machine agreement (soft ship). t8 is the new
REM   regression baseline: it INCLUDES counter-move, so future experiments
REM   measured vs t8 isolate the new change instead of re-counting the +7.
REM
...
REM   - Run this bat on EACH box -> 1000 games -> huginn_vs_t8_<machine>.pgn
REM   - Pool both PGNs (same engine names, same frozen t8 baseline, same tc,
...
REM probe behaviour matches the TB-enabled t8 baseline binary.
REM
REM Usage: test_huginn_vs_t8.bat [rounds_cap]
...
if not exist "%FC%\huginn_t8.exe" (
    echo ERROR: %FC%\huginn_t8.exe not found.
    echo Build it once from git tag baseline-t8 and copy it into the fastchess folder.
    pause
    exit /b 1
)
...
if exist "%RESULTS%\huginn_vs_t8_%MACHINE%.pgn" del /q "%RESULTS%\huginn_vs_t8_%MACHINE%.pgn"
if exist "%RESULTS%\fastchess_t8_%MACHINE%.log" del /q "%RESULTS%\fastchess_t8_%MACHINE%.log"

echo.
echo [%CPU_NAME%] Running SPRT (elo0=0, elo1=10, alpha=0.05, beta=0.05)
echo rounds cap: %ROUNDS% (= up to %ROUNDS%*2 games): Huginn current vs huginn_t8 tc=10+0.1
echo results -^> gauntlet\huginn_vs_t8_%MACHINE%.pgn
echo.

"%FASTCHESS%" ^
  -engine cmd="%FC%\huginn.exe" name=Huginn_current option.OwnBook=false ^
  -engine cmd="%FC%\huginn_t8.exe" name=Huginn_t8 option.OwnBook=false ^
  -each tc=10+0.1 ^
  ...
  -pgnout file="%RESULTS%\huginn_vs_t8_%MACHINE%.pgn" notation=san append=false ^
  -log file="%RESULTS%\fastchess_t8_%MACHINE%.log" level=warn
