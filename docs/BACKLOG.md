...
| 31 | TT-size (`Hash`) sweep (road-to-2.3 item 4) | **DESIGNED (2026-07-14)** — three-engine LTC round-robin (`test_huginn_hash_sweep.bat`): one `huginn_t34.exe` × Hash 64/256/1024, 60+0.6, 600g, run on BOTH boxes (7800X3D's 96MB L3 V-Cache vs Intel — TT probing is memory-latency-bound, optimal size may differ per architecture; PEXT precedent). Decision: clear LTC ordering → bump the UCI release default ([uci.cpp:250](../src/uci.cpp#L250)) for v2.3; gauntlet standard stays 64MB for t-ladder comparability; per-box disagreement → robust middle. **Intel leg (2026-07-15): FLAT** — h1024 +6.95 ± 21.95 / h64 +4.34 ± 21.49 / h256 −11.30 ± 22.78 (600g, 6h25m): non-monotonic ordering inside overlapping CIs = three-way tie; no evidence to move the default on this box. AMD leg pending (the V-Cache box is the one that could differ) | tuning | medium |
