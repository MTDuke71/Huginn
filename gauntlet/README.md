...
|---|---|---:|---|---|
| 2026-06-22 | Intel 13700K | **1000 (SPRT)** | **+3.82 ± 17.52 Elo vs t19**, LOS 66.56%, W334/L323/D343 | **#43 sub-lever 2 — depth-scaled NMP reduction** (candidate = `baseline-t19` + `ENABLE_SCALED_NMP_R` ON: flat null-move `R=4` → `R = 2 + depth/4`, `+1` when `static_eval − beta ≥ 200`, clamped to leave ≥0 child depth). SPRT `elo0=0 elo1=10`, LLR **−0.02 (−0.8%)** → inconclusive at cap, the flat neutral-change shape. CI [−13.7, +21.3] straddles zero; nElo +4.70, score 50.55%, DrawRatio 34.40%, PairsRatio 1.04. Pentanomial [53,108,172,109,58] — near-symmetric, faint positive wing (58 WW vs 53 LL, 109 W-pair vs 108 L-pair), WL/DD **1.73** (decisive-but-level pairs outnumber double-draws — a sharper matchup than a quiet perf batch). Premise was that flat `R=4` over-prunes near the leaf (MTLChess uses `R=2/3` depth-scaled yet is stronger); the Intel leg shows no measurable conversion. **Intel leg neutral; AMD leg pending** → KEEP-OR-REVERT pending the AMD pool. Artifacts: `gauntlet/huginn_vs_t19_intel.pgn` + `gauntlet/fastchess_t19_intel.log`. |
|---|---|---:|---|---|
| 2026-06-18 | Intel 13700K | **1000 (SPRT)** | **+10.43 ± 17.39 Elo vs t18**, LOS 88.03%, W338/L308/D354 | **#9 round 9 — safe mobility** (candidate = `baseline-t18` + `ENABLE_SAFE_MOBILITY` ON, 8 re-tuned per-piece mobility weights). SPRT `elo0=0 elo1=10`, LLR **+0.65 (22.2% to H1)** → inconclusive at cap but positive. nElo +12.93; score 51.50%, DrawRatio 32.60%, PairsRatio 1.16. Pentanomial [50,106,163,126,55], WL/DD 1.67. **AMD leg +5.91 ± 17.81, LOS 74.2%** (1000g SPRT vs t18, W349/L332/D319) — also positive but below 95%. Neither leg clears the 95% bar, but **both legs same-sign positive with tight agreement (+6 to +10)** → clears the cross-machine-agreement ship bar (cf. #15 at 91%) → **SHIP as `baseline-t19`**. Pooled W687/L640/D673 = 51.18% / 2000g. The MSE drop (0.057102→0.056857) did not fully convert — bulk was the joint re-fit, not the term (corroborates #41: eval breadth isn't the gap). Intel artifacts: `gauntlet/huginn_vs_t18_intel.pgn` + `gauntlet/fastchess_t18_intel.log`; AMD: `gauntlet/safemob_vs_t18_amd.pgn`. |
|---|---|---:|---|---|
| 2026-06-17 | Intel 13700K | **1000 (SPRT)** | **+10.08 ± 17.17 Elo vs t17**, LOS 87.54%, W346/L317/D337 | **#43 sub-lever 3 — mate-distance pruning** (candidate = `baseline-t17` + `ENABLE_MATE_DISTANCE_PRUNING` ON; shipped commit `ab37a0d`). SPRT `elo0=0 elo1=10`, LLR **+0.64 (21.6% to H1)** → inconclusive on the Intel leg at cap but clean positive. CI [−7.1, +27.3]; score 51.45%, DrawRatio 35.40%, PairsRatio 1.04. Pentanomial [41,117,177,102,63] — **positive wing skew** (63 WW vs 41 LL, 102 W-pair vs 117 L-pair near-even but the WW wing carries it), WL/DD **2.00** (sharp, decisive matchup — MDP converting mate lines, not a drawish quiet one). **AMD leg +14.95 ± 17.56, LOS 95.28%** (1000g, isolation SPRT vs t17, both arms with #44) — clears the 95% LOS bar; tight cross-machine agreement → **SHIP as `baseline-t18`**. Node-for-node identical to t17 outside mate lines (provably sound: same moves/scores on/off); the gain is faster mate conversion (helps the #5 under-conversion weakness). Default flipped ON; OFF arm kept for ablation. AMD artifacts: `gauntlet/mdpon_vs_t17_amd.pgn`. |
|---|---|---:|---|---|
| 2026-06-03 | Intel 13700K | **1000 (SPRT)** | **−2.43 ± 15.01 Elo vs t10**, LOS 37.53%, W250/L257/D493 | **#35 Exp 2 tapered material values** (HEAD `3b498f9` = `baseline-t10` + phase-blended material values on top of the Part 1 PST taper). SPRT `elo0=0 elo1=10`, LLR **−0.70 (−23.9% to H0)** drifting → inconclusive at cap, the neutral-change shape. CI [−17.4, +12.6] straddles zero; score 49.65%, DrawRatio 41.40%, PairsRatio 0.98. Pentanomial [34, 114, 207, 115, 30] — **near-symmetric, no wing skew** (30 WW vs 34 LL, 115 W-pair vs 114 L-pair). **Dead heat, slight negative lean.** Part 1's +35 is already baked into the t10 baseline; the material-value taper adds nothing measurable on top — likely redundant with the PST taper. **Verdict: KEEP-OR-REVERT pending AMD pool** (running tonight); needs AMD positive to pool to anything shippable. |
|---|---|---:|---|---|
| 2026-06-03 | Intel 13700K | **846 (SPRT H1)** | **+35.03 ± 16.91 Elo vs t9**, **LOS 100%**, W273/L188/D385 | **#35 tapered eval foundation** (HEAD `476d33c`, branch `experiment/tapered-eval` = `baseline-t9` + smooth phase-blended eval). SPRT `elo0=0 elo1=10`, **LLR 2.97 crossed the +2.94 upper bound → H1 ACCEPTED** (stopped early at 846/1000 in 71 min) — the **first H1 accept this session**, not a drift-to-cap. CI [+18.1, +51.9] entirely above zero; score 55.02%, DrawRatio 40.66%, PairsRatio 1.79. Pentanomial [26, 64, 172, 121, 40] — strong positive wing skew (40 WW vs 26 LL, 121 W-pair vs 64 L-pair). A genuine, sizable strength gain — far above any speed-only batch. AMD half reported similar; awaiting AMD SPRT run (tonight) to pool and confirm ship. |
|---|---|---:|---|---|
| 2026-06-02 | AMD 7800X3D | **1000 (SPRT)** | **−6.95 ± 14.19 Elo vs t9**, LOS 16.83%, W214/L234/D552 | **Qsearch delta pruning** (HEAD `a2ee961` = `baseline-t9` + skip captures whose max material swing can't reach alpha, on top of bbef18c PV-display fixes). SPRT `elo0=0 elo1=10`, LLR **−1.29 (−43.8% to H0)** drifting → inconclusive at cap but H0-leaning. CI [−21.1, +7.2] straddles zero with negative center; pentanomial [30, 119, 210, 123, 18] — **negative wing skew** (30 worst-pairs > 18 best-pairs, 119 L-pair > 123 W-pair near-even). Score 49.00%, PairsRatio 0.95, WL/DD 0.35 (heavy DD inside level bucket → near-peer drawish). **Verdict: KEEP-OR-REVERT pending Intel pool.** Lean negative on AMD; if Intel doesn't pull it positive, revert delta pruning — the prune may be too aggressive in late-mg/eg quiet positions where the eval drifts > swing estimate. |
| 2026-06-02 | Intel 13700K | **1000 (SPRT)** | **+3.47 ± 14.55 Elo vs t9**, LOS 68.02%, W233/L223/D544 | **Qsearch delta pruning** (HEAD `a2ee961`, same binary as the AMD row above). SPRT `elo0=0 elo1=10`, LLR +0.01 → flat/inconclusive at cap. Pentanomial [28, 115, 200, 133, 24] — slight positive lean (133 W-pair > 115 L-pair), score 50.50%, PairsRatio 1.10, DrawRatio 40.0%. **POOLED (2000g): ≈ −1.7 Elo** — Intel +3.47 / AMD −6.95 disagree in sign, both inside ±14 noise; pooled pentanomial [58, 234, 410, 256, 42] near-symmetric. **Neutral, within margin of error.** **WAC300 cross-check: identical 270/300 = 90.0%, same 30 failures** (only WAC.055 reshuffled its already-wrong move f4f2→f4f1) → no tactical regression, no over-pruning. **DECISION: KEEP** (overrides the AMD row's lean-revert). Pure speed (−8.6% nodes @ d11), no measurable Elo at 10+0.1 but zero tactical cost; standard qsearch technique that compounds at longer TC — same basis the Syzygy probe was kept on. `DELTA_MARGIN=200` retained (WAC shows no over-pruning to motivate a larger margin). |
|---|---|---:|---|---|
| 2026-06-02 | AMD 7800X3D | **1000 (SPRT)** | **−19.48 ± 14.88 Elo vs t9**, LOS 0.50%, W208/L264/D528 | **BACKLOG #3 weight sweep: CONTHIST_ORDER_WEIGHT 16 → 64** (HEAD `95a8e5e` = `5618b28` + WEIGHT bump only). SPRT `elo0=0 elo1=10`, LLR **−2.72 (92.5% to H0)** → essentially rejected at cap (bound is −2.94). CI [−34, −5]; pentanomial [34, 138, 206, 94, 28] — clean negative skew (34 worst > 28 best, 138 L-pair > 94 W-pair). **26-Elo swing vs WEIGHT=16's +6.60.** Confirms search.hpp's hypothesis: conthist sees 1 parent context (noisier); butterfly sees all (smoother). Letting conthist dominate via high weight overrides the stronger signal. **WEIGHT=16 stays as the tuned value; 64 reverted.** |
|---|---|---:|---|---|
| 2026-06-02 | AMD 7800X3D | **1000 (SPRT)** | **+6.60 ± 14.53 Elo vs t9**, LOS 81.37%, W255/L236/D509 | **BACKLOG #3** 1-ply continuation history (HEAD `5618b28`, `ENABLE_CONTINUATION_HISTORY=1` default, WEIGHT=16 / CAP=8000) on top of `baseline-t9` + Tier 1-4 dead-code cleanup (Elo-neutral). SPRT `elo0=0 elo1=10`, LLR +0.40 (13.5% to H1) → inconclusive at cap, drifting H1 direction. CI [−8, +21]; pentanomial [23,123,192,136,26] — clean positive lean (26 best vs 23 worst, 136 W-pair vs 123 L-pair, DR 38.4%). Same shape as BACKLOG #15 counter-move @1500 (+6.95 LOS 83% AMD → +9.38 LOS 90% Intel → pooled +7.1 SHIP). Awaiting Intel pool. |
|---|---|---:|---|---|
