...
void Position::update_zobrist_for_move(const Move& m, Piece moving, Piece captured) {
    ...
    zobrist_key ^= Zobrist::Piece[piece_index][m.from];
    ...
    if (!is_none(captured)) {
        ...
        zobrist_key ^= Zobrist::Piece[captured_index][m.to];
    }
    
    // XOR in the piece at its new square
    if (m.promo != PieceType::None) {
        // For promotion, the piece type changes
        int promoted_index = int(m.promo) + (moving_color == Color::Black ? 6 : 0);
        zobrist_key ^= Zobrist::Piece[promoted_index][m.to];
    } else {
        // Regular move - same piece type
        zobrist_key ^= Zobrist::Piece[piece_index][m.to];
    }
