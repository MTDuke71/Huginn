...
// VICE Part 77: Pawn structure evaluation using bitboard masks (3:57)
namespace PawnStructure {

// Check if a pawn on the given square is isolated
bool is_isolated_pawn(const Position& pos, int sq64, Color color) {
    if (sq64 < 0 || sq64 > 63) return false;
    
    int file = sq64 % 8;
    uint64_t pawn_bitboard = pos.get_pawn_bitboard(color);
    
    // Check if there are any friendly pawns on adjacent files
    uint64_t adjacent_files_mask = EvalParams::ISOLATED_PAWN_MASKS[file];
    
    return (pawn_bitboard & adjacent_files_mask) == 0;
}

// Check if a pawn on the given square is passed
bool is_passed_pawn(const Position& pos, int sq64, Color color) {
    if (sq64 < 0 || sq64 > 63) return false;
    
    int file = sq64 % 8;
    int rank = sq64 / 8;
    
    uint64_t enemy_pawns = (color == Color::White) ? pos.get_black_pawns() : pos.get_white_pawns();
    
    // Create a mask for the squares that must be clear for this pawn to be passed
    uint64_t passed_mask = 0ULL;
    
    if (color == Color::White) {
        // For white pawns, check ranks ahead and adjacent files
        for (int r = rank + 1; r <= 7; ++r) {
            // Same file
            passed_mask |= (1ULL << (r * 8 + file));
            // Adjacent files if they exist
            if (file > 0) passed_mask |= (1ULL << (r * 8 + file - 1));
            if (file < 7) passed_mask |= (1ULL << (r * 8 + file + 1));
        }
    } else {
        // For black pawns, check ranks behind and adjacent files  
        for (int r = rank - 1; r >= 0; --r) {
            // Same file
            passed_mask |= (1ULL << (r * 8 + file));
            // Adjacent files if they exist
    ...
    return (enemy_pawns & passed_mask) == 0;
}

// Evaluate pawn structure using the new masks
int evaluate_pawn_structure(const Position& pos) {
    ...
    for (int sq64 = 0; sq64 < 64; ++sq64) {
        ...
        if (piece == Piece::WhitePawn) {
            if (is_isolated_pawn(pos, sq64, Color::White)) {
                score -= EvalParams::ISOLATED_PAWN_PENALTY;
            }
            if (is_passed_pawn(pos, sq64, Color::White)) {
                int rank = sq64 / 8;
                score += EvalParams::PASSED_PAWN_BONUS[rank];
            }
        } else if (piece == Piece::BlackPawn) {
            if (is_isolated_pawn(pos, sq64, Color::Black)) {
                score += EvalParams::ISOLATED_PAWN_PENALTY; // Penalty for black
            }
            if (is_passed_pawn(pos, sq64, Color::Black)) {
                int rank = 7 - (sq64 / 8); // Mirror rank for black
                score -= EvalParams::PASSED_PAWN_BONUS[rank]; // Bonus for black
            }
        }
    }
