#!/usr/bin/env python3
"""
LCT II (Louguet Chess Test II) Test Suite for Huginn Chess Engine
Tests engine tactical/positional ability against the 35-position LCT2 suite
(test/lct2.epd) — real master-game positions, split into positional
(LCTII.POS.*), combinational (LCTII.CMB.*), and endgame (LCTII.FIN.*)
categories. Same engine harness and output format as wac_test.py /
wac201_test.py; only the default EPD file + position cap differ.
"""
...
class WACTester:
    def __init__(self, engine_path: str = "../build/msvc-x64-release/bin/Release/huginn.exe",
                 epd_file: str = "lct2.epd", max_positions: Optional[int] = None,
    ...
    def run_test_suite(self, search_time: int = 5):
        ...
        log_filename = f"lct2_test_log_{timestamp}.txt"

        with open(log_filename, 'w') as self.log_file:
            self.log_file.write(f"LCT II Test Suite - Huginn Chess Engine\n")
    ...
    def generate_summary(self, log_filename: str):
        ...
        print(f"\n{'='*60}")
        print("LCT II TEST SUITE SUMMARY")
        print(f"{'='*60}")
        ...
        if failed > 0:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            failed_file = f"lct2_failed_positions_{timestamp}.txt"
            with open(failed_file, 'w') as f:
                f.write("# Failed LCT2 position line numbers\n")
                f.write("# Use: python lct2_test.py --failed-file lct2_failed_positions_XXXXXX.txt\n")
    ...
    def retest_single_position(self, failed_idx: int):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        retest_log = f"lct2_retest_{position.id.replace('.', '_')}_{timestamp}.txt"

        with open(retest_log, 'w') as self.log_file:
            self.log_file.write(f"LCT II Retest - {position.id}\n")
            self.log_file.write(f"Timestamp: {datetime.now()}\n")
    ...
    def retest_all_failed(self):
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        retest_log = f"lct2_retest_all_{timestamp}.txt"

        updated_count = 0

        with open(retest_log, 'w') as self.log_file:
            self.log_file.write(f"LCT II Batch Retest\n")
            self.log_file.write(f"Timestamp: {datetime.now()}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='LCT II Test Suite for Huginn Chess Engine',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python lct2_test.py             # Full suite (35 positions), 5 seconds each
  python lct2_test.py 10          # First 10 positions, 5 seconds each
  python lct2_test.py 35 -t 10    # Full suite, 10 seconds each
  python lct2_test.py --failed-file lct2_failed_positions_20260715_123456.txt
                                  # Retest only previously failed positions

Runs lct2.epd (Louguet Chess Test II — 35 positional/combinational/endgame
master-game positions). The script automatically offers interactive retest
mode for failed positions; failed positions are saved to
lct2_failed_positions_TIMESTAMP.txt for easy re-testing.
        """)
    parser.add_argument('positions', nargs='?', type=int, default=35,
                       help='Number of positions to test (default: 35 = full set, max: 35)')
    ...
    if args.failed_file:
        ...
        print(f"LCT II Test Suite - Retesting failed positions from {args.failed_file}")
        ...
        # Normal mode
        if args.positions <= 0 or args.positions > 35:
            print("Error: Number of positions must be between 1 and 35")
            return
        ...
        print(f"LCT II Test Suite - Testing first {args.positions} positions")
        print(f"Each position will be searched for {args.time} seconds")
