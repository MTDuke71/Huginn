...
// OPTIMIZATION V6: Comprehensive fast move generation with reduced overhead
namespace FastMoveGen {
    ...
    inline void generate_bishop_moves_fast(const Position& pos, S_MOVELIST& list, Color us) {
        ...
        while (bishops != 0) {
            ...
            // Process captures first (potentially better for move ordering)
            while (captures != 0) {
                int to_sq64 = get_lsb(captures);
                captures &= captures - 1;
                int to_sq120 = MAILBOX_MAPS.to120[to_sq64];
    ...
    inline void generate_rook_moves_fast(const Position& pos, S_MOVELIST& list, Color us) {
    ...
    inline void generate_all_moves_optimized(const Position& pos, S_MOVELIST& list, Color us) {
        ...
        // Use optimized generation for major pieces
        generate_knight_moves_fast(pos, list, us);
        generate_bishop_moves_fast(pos, list, us);
        generate_rook_moves_fast(pos, list, us);
