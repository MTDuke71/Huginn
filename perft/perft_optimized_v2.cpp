...
// Huginn2 Perft function - OPTIMIZED V2: Reduce move validation overhead
static uint64_t perft_huginn2(Position& pos, int depth) {
    ...
    // OPTIMIZATION V2: Continue with inlined move generation
    ...
    // OPTIMIZATION V2: Use more efficient iteration and reduce branching
    const S_MOVE* moves = list.moves;
    const int move_count = list.count;
    
    for (int i = 0; i < move_count; ++i) {
        // Use VICE MakeMove/TakeMove for validation - but cache the result
        if (pos.MakeMove(moves[i])) {  // returns 1 for legal, 0 for illegal
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== Huginn2 Perft Suite OPTIMIZED V2 (Reduced Validation) ===" << std::endl;
    std::cout << "Testing optimized bitboard move generation with reduced validation overhead" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "  Method: Huginn2 optimized bitboard move generation (V2)" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Will run approximately " << total_expected_tests << " tests using Huginn2 optimized bitboard generation (V2)" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Test method: Huginn2 optimized bitboard move generation (V2)" << std::endl;
