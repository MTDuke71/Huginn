...
| 59 | En-passant key semantics (repetition + Polyglot) | **FIXED on main (2026-07-11)** — EP right normalized at source (MakeMove + set_from_fen, X-FEN convention); Polyglot wrong-rank check replaced, spec anchor keys pass; unconditional (#50 precedent); AMD regression gate CLEAN: +8.34 ± 15.32, LOS 85.73%, 1000g cap (`gauntlet/huginn_vs_t28_ep_amd.pgn`) — t29 tag pending user call | bug/rules/book | high |
