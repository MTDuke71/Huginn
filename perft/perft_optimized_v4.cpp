...
// Huginn2 Perft function - OPTIMIZED V4: Bulk move validation to reduce overhead
static uint64_t perft_huginn2(Position& pos, int depth) {
    ...
    // OPTIMIZATION V4: Enhanced depth 1 handling with bulk processing
    if (depth == 1) {
        uint64_t legal_moves = 0;
        const S_MOVE* moves = list.moves;
        const int move_count = list.count;
        
        // Process moves in bulk for better cache locality
    ...
    // OPTIMIZATION V4: Unroll loop for small move counts (common case)
    uint64_t nodes = 0;
    const S_MOVE* moves = list.moves;
    const int move_count = list.count;
    
    // Unroll for very small move counts (castling only, endgame positions)
    if (move_count <= 4) {
        for (int i = 0; i < move_count; ++i) {
            if (pos.MakeMove(moves[i])) {
        ...
        // Standard loop for normal positions
        for (int i = 0; i < move_count; ++i) {
            if (pos.MakeMove(moves[i])) {
                nodes += perft_huginn2(pos, depth - 1);
                pos.TakeMove();
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "=== Huginn2 Perft Suite OPTIMIZED V4 (Bulk Validation) ===" << std::endl;
    std::cout << "Testing optimized perft with bulk validation and loop unrolling" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "  Method: Huginn2 optimized perft with bulk validation (V4)" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Will run approximately " << total_expected_tests << " tests using optimized perft with bulk validation (V4)" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "Test method: Huginn2 optimized perft with bulk validation (V4)" << std::endl;
